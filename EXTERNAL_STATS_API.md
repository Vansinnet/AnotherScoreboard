...
- Active-run export includes external scalar maps, including explicit zero and text. Import validates external data before changing Stats. Values for unknown providers/rows remain pending across export/import until the matching stat registers. A saved value-type mismatch is discarded. Importing an old active run without external data yields empty external values.
